...
namespace Castle.DynamicProxy.Extensions.Test.ServiceClass
{
    public class LoggerAttribute: AbstractInterceptorAttribute
    {
        ...
        {
            Debug.WriteLine($"LoggerAttribute执行[{context.Method.Name}]前,param.length=[{context.Parameters.Length}]");
            await next(context);
            ...
            if (returnValueType.IsAsync())
            {
                ...
                {
                    Debug.WriteLine($"LoggerAttribute执行[{context.Method.Name}]后,returnValue=[{context.ReturnValue}]");
                    return;
                }
                ...
                if (returnValueType.IsTaskWithResult())
                {
                    ...
                    Debug.WriteLine($"LoggerAttribute执行[{context.Method.Name}]后,returnValue=[{returnValue}]");
                    return;
                }
